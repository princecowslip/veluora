...
- Only 3 of the 12 connector types `docs/46-implementation-plan.md`'s Workstream 10 names as "implement first" exist: a local-filesystem wrapper, an RSS/Atom feed connector, and a generic Danbooru/Gelbooru-compatible booru connector (Milestone K — one `flavor`-configured implementation covers both the Danbooru-family and Gelbooru-family line items, not two separate connectors). OPDS, Stash, Hydrus, Jellyfin, Kavita, Komga, a metadata-only Stash-box connector, and a browser-handoff template are all still unbuilt — most need real service accounts/API keys unavailable in the environment this was built in. `FeedConnector` and `BooruConnector` are the only download-capable connectors; the local-filesystem connector's items are already local, and none of the remaining unbuilt connector types can exercise downloads until they exist.
